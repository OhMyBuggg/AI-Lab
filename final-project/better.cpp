// Team 9.cpp : 此檔案包含 'main' 函式。程式會於該處開始執行及結束執行。
//
...
vector<vector<vector<int> > > ExpandChildMove(std::vector<std::vector<int> >& board, int nowx, int nowy);
...
public:
		...
		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 8; j++) {
				if (local_board[i][j] == piece_color)
					children_step = ExpandChildMove(local_board, i, j);
	...
	vector<vector<int> > return_step() {
		...
		for (int i = 0; i < root->children.size(); i++) {
			child_utility = root->children[i]->time;
...
private:
	...
	float select_min_node(tree_node *current_node) {
		...
		vector<int> candidate_child_node;//the nodes have the same good uility
		for (int i = 0; i < current_node->children.size(); i++) {
			child_node_utility = OUCB(current_node->children[i]);
...
int simulation(int round) {
	int ene = self == 1 ? 2 : 1;
	int whose_turn = round % 2 == 0 ? self : opponent;
...
std::vector<std::vector<int>> GetStep(std::vector<std::vector<int>>& board, bool is_black, tree &MCTS) {
	...
	all_sample_time = MCTS.get_root()->time;
	clock_t start, stop, pre_stop , max_time = 0;
	start = clock();
	pre_stop = clock();
	stop = clock();
	if (MCTS.get_root()->depth != 0) {
		MCTS.move_root(board);
		stop = clock();
		max_time = stop - pre_stop;
		pre_stop = stop;
	}
	while (5000 - stop + start > max_time) {
		local_board = board;
		MCTS.MCTS_select();
		stop = clock();
		max_time = max(max_time, stop - pre_stop);
		pre_stop = stop;
	}
	local_board = board;
	step = MCTS.return_step();
	return step;
}
...
int main() {
	...
	while (true) {
		if (GetBoard(id_package, board, is_black))
			break;
		step = GetStep(board, is_black, MCTS);
//		if (step.size()) {
